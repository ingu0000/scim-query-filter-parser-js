...
const WORD = '[\\w\\.]+';
const SEP = '\\s?';
const NEXT_TOKEN = new RegExp(`^(${PAREN}|${STR}|${WORD}|${OP}|${WORD})${SEP}`);
...
class Filter {
  ...
  parseExpr () {
    ...
    while (this.tokens.length && this.tokens[0] !== ')') {
      ...
      if (expectOp && !IS_OPERATOR.test(token.toLowerCase()))
  ...
  static toRpn (ast) {
    ...
    while (ast.length) {
      let op = ast.shift();
      let p = OPS[op.toLowerCase && op.toLowerCase()];
